...
DEFAULT_DIR = '/tmp/mitm-requests'


def response(flow):
    file_path = os.getenv('MITM_DIR', DEFAULT_DIR)
    ...
    HAR.update({
        "log": {
            "version": "1.2",
            "creator": {
                "name": "mitmproxy request har dump",
                "version": "0.1",
    ...
    with open(f"/tmp/mitm-requests/{str(uuid.uuid4())}", "wb") as f:
        f.write(raw)
